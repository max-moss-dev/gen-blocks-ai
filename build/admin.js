...
jQuery(document).ready(function ($) {
  ...
  $('<div class="shortcut-info">Press Enter to send, Shift+Enter for new line</div>').insertAfter(chatTextarea).css({
    'font-size': '12px',
    'color': '#666',
    'margin-top': '4px'
  });
  ...
  function addMessage(content, role) {
    const message = $('<div>').addClass('chat-message').addClass(role).text(content);
  ...
  $('#block-generation-form').on('submit', function (e) {
    ...
    if (selectedImage) {
      ...
      const imageFile = new File([new Blob([''], {
        type: 'image/jpeg'
      })], 'image.jpg', {
        type: 'image/jpeg'
      });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          let html = template.replace(/<style>[\s\S]*?<\/style>/, '').replace(/<script>[\s\S]*?<\/script>/, '').trim();
  ...
  function showError(message) {
    const notice = $('<div class="notice notice-error is-dismissible"><p></p><button type="button" class="notice-dismiss"></button></div>').find('p').text(message).end();
  ...
  function showSuccess(message) {
    const notice = $('<div class="notice notice-success is-dismissible"><p></p><button type="button" class="notice-dismiss"></button></div>').find('p').text(message).end();
  ...
  function initializeIconPicker() {
    ...
    if (iconPickerContainer.length && !iconPickerContainer.html().trim()) {
      const dashicons = ['admin-appearance', 'admin-comments', 'admin-home', 'admin-media', 'admin-page', 'admin-post', 'admin-settings', 'admin-users', 'align-center', 'align-left', 'align-right', 'analytics', 'archive', 'arrow-down', 'arrow-left', 'arrow-right', 'arrow-up', 'art', 'awards', 'backup', 'block-default', 'button', 'calendar', 'camera', 'category', 'chart-area', 'chart-bar', 'chart-line', 'chart-pie', 'clipboard', 'clock', 'cloud', 'columns'];
      let iconsHtml = '';
    ...
    iconPickerContainer.find('.icon-option').off('click').on('click', function () {
      ...
      // Update preview
      const iconPreview = $('<span class="dashicons"></span>').addClass('dashicons-' + selectedIcon).css({
        'position': 'absolute',
        'left': '8px',
        'top': '50%',
